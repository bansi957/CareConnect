...
const hospitalSignIn = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Hospital signed in successfully", data: hospital });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Internal server error" });
  }
...
const hospitalSignOut = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Hospital signed out successfully" });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Internal server error" });
  }
...
const getHospitalById = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Hospital found successfully", data: hospital });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Internal server error" });
  }
...
const getAllHospitals = async (req, res) => {
  try {
    const { name, city, state, hospitalCategory } = req.query;
    const filter = {};
    if (name) filter.name = { $regex: name, $options: "i" };
    if (city) filter.city = { $regex: city, $options: "i" };
    if (state) filter.state = { $regex: state, $options: "i" };
    if (hospitalCategory)
      filter.hospitalCategory = { $regex: hospitalCategory, $options: "i" };

    const hospitals = await Hospital.find(filter).select(
      "name image address city state hospitalCategory noOfDoctors availableBeds workingHours status",
    );
    return res
      .status(200)
      .json({ message: "Hospitals fetched successfully", hospitals });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Internal server error" });
  }
